...
- (NSString *)yb_formatDistance{
    ...
    if (Distance < 1000) {
        return [NSString stringWithFormat:@"%@米",self];
    }else{
        CGFloat km =(CGFloat)Distance/1000.0;
        return [NSString stringWithFormat:@"%0.1fkm",km];
    }
}
//专成秒分钟或者时间
- (NSString *)yb_formatDuration{
    NSInteger sec = self.integerValue;
    if (sec < 60) {
        return [NSString stringWithFormat:@"%@秒",self];
    }else{
        NSInteger min = (NSInteger)sec/60;
        if (min < 60) {
            return [NSString stringWithFormat:@"%0.1ld分钟",(long)min];
        }else{
            CGFloat h = (CGFloat)min/60;
            return [NSString stringWithFormat:@"%0.1f小时",h];
        }
    }
}
...
- (NSString *)yb_getYouonSmallImage{
    return [self yb_getYouonSmallImageWithPercent:18];
}
...
- (NSString *)yb_getYouonSmallImageWithPercent:(NSInteger)percent{
    ...
    NSString *temp = [NSString stringWithFormat:@"?x-oss-process=image/quality,q_%ld",(long)percent];
    return [self stringByAppendingString:temp];
}
